...
# --- Read existing ALLOWED_HOSTS and CSRF_TRUSTED_ORIGINS if present ---
EXISTING_ALLOWED_HOSTS=""
EXISTING_CSRF=""
if [ -f .env ]; then
    EXISTING_ALLOWED_HOSTS=$(grep "^ALLOWED_HOSTS=" .env | cut -d'=' -f2-)
    EXISTING_CSRF=$(grep "^CSRF_TRUSTED_ORIGINS=" .env | cut -d'=' -f2-)
fi
...
# --- Restore preserved host configuration ---
if [ -n "$EXISTING_ALLOWED_HOSTS" ]; then
    echo "ALLOWED_HOSTS=$EXISTING_ALLOWED_HOSTS" >> .env
fi
if [ -n "$EXISTING_CSRF" ]; then
    echo "CSRF_TRUSTED_ORIGINS=$EXISTING_CSRF" >> .env
fi
echo ""
